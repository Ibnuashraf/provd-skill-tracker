...
export function SkillsView({ SD, setSD, MS, setMS, setView, observerMode }) {
  ...
  const addNewSkill = async () => {
    if(!newSk.trim()) return;
    setIsGenerating(true);
    
    const id = newSk.toLowerCase().replace(/[^a-z0-9]/g,'-');
    const newObj = { id, nm: newSk, ic: '✨', lv: 1, tgt: 10, ents: 0, pts: 0, stk: 0, bc: 'rgba(184,232,48,.1)', bt: '#B8E830' };
    setSD([...SD, newObj]);
    
    // Call AI to generate 5 sequential milestones
    const newMilestones = await generateMilestones(newSk);
    setMS({ ...MS, [id]: newMilestones });
        ...
        <div>
          ...
          {showAdd && (
            <div className="nsk-form show">
              ...
              <button className="sbmt" style={{padding:'6px 14px',fontSize:'.78rem',opacity: isGenerating ? 0.6 : 1}} onClick={addNewSkill} disabled={isGenerating}>
